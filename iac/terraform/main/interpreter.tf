...
resource "google_cloud_run_v2_service" "interpreter" {
  ...
  template {
    timeout = "10s"
    containers {
      image = "${ var.GCP_PROJECT_REGION }-docker.pkg.dev/${ var.GCP_PROJECT_ID }/${ google_artifact_registry_repository.food-interpreter-repository.name }/food-interpreter-interpreter"
      resources {
        limits = {
          memory = "1024Mi"
          cpu    = "1000m"
...
resource "google_pubsub_subscription" "interpreter" {
  ...
  push_config {
    push_endpoint = "${ google_cloud_run_v2_service.interpreter.uri }/pubsub-interpret"
    oidc_token {
